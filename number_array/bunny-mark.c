...
// } Bunny;
  ...
  void createBunnies(int requested, int* bunniesCount, float* bunny_x, float* bunny_y, float* bunny_sx, float* bunny_sy)
  // void createBunnies(int requested, int* bunniesCount, Bunny* bunnies)
  {
    ...
    for (int i = 0; i < count; i++)
    {
        if (*bunniesCount <= MAX_BUNNIES)
        {
            ...
            *bunny_y = 88.0/2;
            *bunny_sx = 50.0/60.0f;
            *bunny_sy = 250.0/60.0f;

            // bunnies[*bunniesCount].position.x = 50/2;
            // bunnies[*bunniesCount].position.y = 88/2;
            // bunnies[*bunniesCount].speed.x = 50.0/60.0f;
            // bunnies[*bunniesCount].speed.y = 250.0/60.0f;
